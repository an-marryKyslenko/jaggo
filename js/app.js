(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".booking__slider")) {
            new core(".booking__slider", {
                ...
                on: {
                    activeIndexChange: function() {
                        myCollbackFunction(this);
                    }
                }
            });
        }
    }
    function myCollbackFunction(data) {
        let collection = document.querySelectorAll(".item-book");
        for (let i = 0; i < collection.length; i++) {
            const item = collection[i];
            let timing = item.getAttribute("data-swiper-autoplay");
            let interval = timing / 100;
            let itemBox = item.lastElementChild;
            if (item.getAttribute("data-swiper-slide-index") == data.realIndex) {
                let itemNext = collection[data.realIndex];
                let title = itemNext.querySelector(".item-book__title").innerHTML;
                let img = itemNext.querySelector(".item-book__img").innerHTML;
                if (!itemBox.classList.contains("box")) {
                    item.insertAdjacentHTML("beforeend", `<div class="box">\n\t\t\t\t\t<div class="box__img">\n\t\t\t\t\t\t${img}\n\t\t\t\t\t</div>\n\t\t\t\t\t<div class="box__contant">\n\t\t\t\t\t\t<p class="box__subtitle">Ongoing</p>\n\t\t\t\t\t\t<p class="box__title">${title}</p>\n\t\t\t\t\t\t<p class="box__text-loaded"><span class="box__procent"><span id="procent"></span>%</span>  completed</p>\n\t\t\t\t\t\t<div class="box__loaded"></div>\n\t\t\t\t\t</div>\n\t\t\t\t</div>`);
                    let prevItem = collection[data.previousIndex].lastElementChild;
                    if (prevItem.classList.contains("box")) prevItem.remove();
                    let procentLoad = item.querySelector("#procent");
                    let load = 0;
                    function updataLoader() {
                        load += load < 100;
                        procentLoad.innerHTML = load;
                    }
                    setInterval(updataLoader, interval);
                }
            }
        }
    }
